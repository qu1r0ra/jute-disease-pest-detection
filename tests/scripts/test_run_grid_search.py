...
def test_run_grid_search_phase1(
    ...
    assert any("drop_rate=0.1" in c for c in cmd)
    assert any("pretrained=True" in c for c in cmd)
    assert any("checkpoint_path=null" in c for c in cmd)
...
def test_run_grid_search_phase2(
    ...
    assert "fit" in cmd
    assert "--config" in cmd
    assert any("drop_rate=0.3" in c for c in cmd)
    assert any("pretrained=False" in c for c in cmd)
    ...
    assert any(expected in c for c in cmd)
    assert any("lr=0.005" in c for c in cmd)
    assert any("weight_decay=0.1" in c for c in cmd)
